...
uint256 hashGenesisBlock("0xaf470de35699534224a92cb04a9475fe386cbb88b88c5d17a1bc9bda27d240b8");
...
const string strMessageMagic = "PAPAFRANCESCOCoin Signed Message:\n";
...
{
    // PAPAFRANCESCOCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // PAPAFRANCESCOCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1467650464;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5038361;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x18c372fe354185169070e1c23188f3d028271d8445e1be02f50db9aa353ed056"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xce, 0xaa, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PAPAFRANCESCOCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PAPAFRANCESCOCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PAPAFRANCESCOCoinMiner(CWallet *pwallet)
{
    printf("PAPAFRANCESCOCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PAPAFRANCESCOCoin-miner");
    ...
    try { loop {
        ...
        printf("Running PAPAFRANCESCOCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PAPAFRANCESCOCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PAPAFRANCESCOCoinMiner, pwallet));
}
